...
namespace LibXR {
/**
 * @brief QTTimebase 类，用于获取 Qt 系统的时间基准。Provides a timebase for Qt
 * systems.
 *
 */
...
public:
  /**
   * @brief 获取当前时间戳（微秒级）。Returns the current timestamp in
   * microseconds.
   *
   * @return TimestampUS
   */
  ...
  /**
   * @brief 获取当前时间戳（毫秒级）。Returns the current timestamp in
   * milliseconds.
   *
   * @return TimestampMS
   */
